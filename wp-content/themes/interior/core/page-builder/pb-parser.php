    ...
    foreach ($modules as $module_key => $module) {
        ...
        if ($module['name']=="bg_start" || $module['name']=="bg_end") {
            if ($module['name']=="bg_start") {
                echo '<div class="module_line_trigger" data-option="'.$module['properties'].'" data-background="'.((strlen($module['bg_color'])>0) ? "#".$module['bg_color'] : '').' '.((strlen($module['bg_image'])>0) ? "url('".$module['bg_image']."')" : '').' '.((strlen($module['properties']) == "stretch") ? 'no-repeat' : 'repeat').' 0 0" data-top-padding="'.$module['padding_top'].'" data-bottom-padding="'.$module['padding_bottom'].'">';
            }
            if ($module['name']=="bg_end") {
                if ($lineWidth<1) {
                    echo '</div><!-- bg_end here -->';
                }
            ...
            echo "<div class='{$outputClass} ".(($normalModulesICounter==1) ? "first-module" : "")." module_number_{$normalModulesICounter} module_cont ".(($module['heading_alignment'] == "center") ? 'center_title' : '')." {$module['padding_bottom']} module_{$module['name']}'>";
