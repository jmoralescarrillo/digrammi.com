...
<div id="comments">
    ...
    <?php if (have_comments()) : ?>
        <div class="bg_title"><h4 class="headInModule postcomment"><?php echo ((get_theme_option("translator_status") == "enable") ? get_text("comments_number") : __('Comments','theme_localization')).": "; echo comments_number( '0', '1', '%' ); ?></h4></div>
    ...
    $comment_form = array(
        'fields' => apply_filters( 'comment_form_default_fields', array(
            'author' => '<div><label class="label-name"></label><input type="text" value="'.((get_theme_option("translator_status") == "enable") ? get_text("comment_form_name") : __('Name *','theme_localization')).'" title="'.((get_theme_option("translator_status") == "enable") ? get_text("comment_form_name") : __('Name *','theme_localization')).'" id="author" name="author" class="form_field"></div>',
            'email'  => '<div><label class="label-email"></label><input type="text" value="'.((get_theme_option("translator_status") == "enable") ? get_text("comment_form_email") : __('Email *','theme_localization')).'" title="'.((get_theme_option("translator_status") == "enable") ? get_text("comment_form_email") : __('Email *','theme_localization')).'" id="email" name="email" class="form_field"></div>',
            'url'    => '<div><label class="label-web"></label><input type="text" value="'.((get_theme_option("translator_status") == "enable") ? get_text("comment_form_url") : __('URL','theme_localization')).'" title="'.((get_theme_option("translator_status") == "enable") ? get_text("comment_form_url") : __('URL','theme_localization')).'" id="web" name="url" class="form_field"></div>'
        ) ),
        'comment_field' => '<div><label class="label-message"></label><textarea name="comment" cols="45" rows="5" title="'.((get_theme_option("translator_status") == "enable") ? get_text("comment_form_message") : __('Message...','theme_localization')).'" id="comment-message" class="form_field">'.((get_theme_option("translator_status") == "enable") ? get_text("comment_form_message") : __('Message...','theme_localization')).'</textarea></div>',
        'comment_form_before' => '',
        'comment_form_after' => '',
